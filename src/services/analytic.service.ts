...
// Monthly income/expense trend for a single category over a year.
async function getCategoryTrend(accessToken: string, userId: string, categoryId: string, year: number) {
    const db = supabaseForUser(accessToken)
    const start = `${year}-01-01`
    const end = `${year + 1}-01-01`
    const { data, error } = await db
        .from('expenses')
        .select('direction, amount, expense_date, categories (name)')
        .eq('user_id', userId)
    ...
    for (const row of (data ?? []) as any[]) {
        categoryName = row.categories?.name ?? categoryName
        const m = Number(String(row.expense_date).slice(5, 7)) - 1
        if (m < 0 || m > 11) continue
        const amount = Number(row.amount) || 0
        if (row.direction === 'in') months[m]!.income += amount
    ...
    return { category_id: categoryId, category_name: categoryName, year, months }
}
